...
exports.verifyEmail = (0, awaitHandlerFactory_middleware_1.default)(async (req, res) => {
    ...
    try {
        ...
        const loginToken = jsonwebtoken_1.default.sign({ userId: result[0].id, email: result[0].email }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRE || '7d' });
        res.status(200).json({
            statusCode: 200,
            success: true,
...
exports.resendVerification = (0, awaitHandlerFactory_middleware_1.default)(async (req, res) => {
    const { email } = req.body;
    if (!email) {
        return res.status(400).json({
            statusCode: 400,
...
exports.socialCallback = (0, awaitHandlerFactory_middleware_1.default)(async (req, res) => {
    const user = req.user;
    if (!user) {
